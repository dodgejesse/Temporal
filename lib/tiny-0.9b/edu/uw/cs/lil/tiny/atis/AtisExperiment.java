...
import edu.uw.cs.lil.tiny.atis.parser.ccg.rules.typeshifting.APComposition;
import edu.uw.cs.lil.tiny.atis.parser.ccg.rules.typeshifting.PPComposition;
import edu.uw.cs.lil.tiny.atis.parser.ccg.rules.typeshifting.PluralAPTypeShifting;
import edu.uw.cs.lil.tiny.atis.parser.ccg.rules.typeshifting.PluralPPTypeShifting;
import edu.uw.cs.lil.tiny.atis.parser.ccg.rules.typeshifting.ThatlessRelative;
...
import edu.uw.cs.lil.tiny.paser.ccg.rules.lambda.typeshifting.templated.ForwardTypeRaisedComposition;
...
public class AtisExperiment extends DistributedExperiment {
	...
	private final ResourceCreatorRepository			resCreatorRepo			= new AtisResourceRepo();
	
	public AtisExperiment(File initFile) throws IOException, SAXException {
		super(initFile);
		...
		final Set<List<String>> genlexIgnore = new HashSet<List<String>>();
		for (final LexicalEntry<LogicalExpression> entry : unfactoredLexicon
				.toCollection()) {
			genlexIgnore.add(entry.getTokens());
		}
		storeResource("genlexIgnore", genlexIgnore);
		...
		ruleSetBuilder.add("shift_pl_pp", new PluralPPTypeShifting(
				categoryServices));
		ruleSetBuilder.add("shift_pl_ap", new PluralAPTypeShifting(
				categoryServices));
		...
		final AbstractCKYParser<LogicalExpression> parser = new MultiCKYParser.Builder<LogicalExpression>(
				categoryServices, this, parseFilter)
				.addBinaryParseRule(
				...
				.addBinaryParseRule(
						new CKYBinaryParsingRule<LogicalExpression>(
								new APComposition(categoryServices)))
				.addBinaryParseRule(
						new CKYBinaryParsingRule<LogicalExpression>(
								new ThatlessRelative(categoryServices)))
				.setMaxNumberOfCellsInSpan(parserBeamSize)
				.setPreChartPruning(true).setPruneLexicalCells(true).build();
		...
		return new Job(params.get("id"), new HashSet<String>(
			...
			protected void doJob() {
				try {
					...
					throw new RuntimeException("failed to save model to: "
							+ directory);
				}
			}
		};
