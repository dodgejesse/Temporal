...
public class MultiCKYParser<Y> extends AbstractCKYParser<Y> {
	...
	public static class Builder<Y> {
		...
		private IFilter<Category<Y>>						completeParseFilter			= new SimpleFullParseFilter<Y>(
		...
		public Builder<Y> setCompleteParseFilter(
				IFilter<Category<Y>> completeParseFilter) {
	...
	private class LexicalJob extends AbstractJob {
		...
		public void loggedRun() {
			LOG.debug("Lexical job started: %s", split.span);
			
			final List<Cell<Y>> newCells = generateLexicalCells(split.begin,
					split.end, chart, lexicons);
			
			LOG.debug("%d new lexical cells", newCells.size());
			...
			if (pruneLexicalCells) {
				...
				LOG.debug("%d new lexical cells passed hard pruning",
						newCells.size());
			}
			...
			LOG.debug("Lexical job completed: %s", split.span);
	...
	private class SplitJob extends AbstractJob {
		...
		public void loggedRun() {
			...
			LOG.debug("%d new cells", newCells.size());
			...
			LOG.debug("Job completed: %s", split);
