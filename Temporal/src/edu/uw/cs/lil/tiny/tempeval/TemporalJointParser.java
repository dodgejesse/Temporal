...
String[], LogicalExpression, LogicalExpression, Pair<String, String>> {
	...
	// To prune the parses that have lambdas. My logical expressions shouldn't have a complex type, or they wont execute.
	...
	/*
	 * To perform the first step of the execution - getting all possible context-dependent logical forms.
	 */
	...
	public IJointOutput<LogicalExpression, Pair<String, String>> parse(
		...
		boolean sameDocID = dataItem.getSample().second()[0].equals(dataItem.getSample().second()[3]) && prevISO != null;
		
		Sentence phrase = dataItem.getSample().first();
		IParserOutput<LogicalExpression> CKYParserOutput = baseParser.parse(phrase, model);
		final List<IParseResult<LogicalExpression>> CKYModelParses = 
				pruneLogicWithLambdas(CKYParserOutput.getBestParses());
		...
		for (IParseResult<LogicalExpression> l : CKYModelParses){
			...
			for (int i = 0; i < labels.length; i++){
				...
				// create a TemporalExecResultWrapper class using the Pair<String, String> and the score.
				// use that wrapper to create 
				String ref_time = dataItem.getSample().second()[2];
				TemporalISO tmp = TemporalVisitor.of(labels[i], ref_time, prevISO);
				...
				IJointParse<LogicalExpression, Pair<String, String>> jp = new JointParse<LogicalExpression,Pair<String, String>>(l, wrapper);
		...
		JointOutput<LogicalExpression, Pair<String, String>> out = new JointOutput<LogicalExpression, Pair<String, String>>(CKYParserOutput, allExecutedParses, parsingTime);
		return out;	
