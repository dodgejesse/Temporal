...
public class TemporalDetection {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		if (CROSS_VAL){
			...
			// to make the threads
			TemporalDetectionThread[] threads = new TemporalDetectionThread[splitData.size()];
			...
			for (int i = 0; i < splitData.size(); i++){
				...
				threads[i] = new TemporalDetectionThread(newTrain, newTest, parser, fixed, lexPhi, i);
				threads[i].start();
			}
			...
			new TemporalDetectionThread(train, test, parser, fixed, lexPhi, -1).run();
		}
		System.out.println("Done");
	}
}
