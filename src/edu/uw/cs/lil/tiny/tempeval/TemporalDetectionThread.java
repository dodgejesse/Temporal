...
public class TemporalDetectionThread extends Thread {
		...
		for (int i = 0 ; i < tokens.size(); i++) {					
			...
			for (int span = Math.min(MAX_MENTION_LENGTH, tokens.size() - i)  ; span > 0 ; span--) {
				...
				if (isoParses.size() > 0) {
					...
					System.out.printf ("Predicted mention [#%d]: '%s' (%s)\n", predictedCharNum, mentionCandidate, isoParses.get(0));
	...
	public void run(){
		System.out.println("About to parse all substrings");
		...
		for (final ILabeledDataItem<Pair<Sentence, String[]>, TemporalResult> dataItem : train) {
			...
			String fullSentence = dataItem.getSample().second()[1].replace("-", " ").replace(","," ").replace("."," ").toLowerCase();
